...
def overwrite_default_python_interpreter(isaaclab_settings: str) -> str:
    ...
    # normalize path separators to forward slashes for JSON compatibility and to avoid regex escape issues
    python_exe_normalized = python_exe.replace("\\", "/")
    isaaclab_settings = re.sub(
        r"\"python.defaultInterpreterPath\": \".*?\"",
        f'"python.defaultInterpreterPath": "{python_exe_normalized}"',
        isaaclab_settings,
        flags=re.DOTALL,
    )
